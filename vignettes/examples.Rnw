...
evolve <- gibbs(reproduce(ladder), mutate(ladder,runif4), p=c(0.7, 0.3))

init <- matrix(runif(t*3),t,3)
lineage <- iterate(1000, evolve, init,bar=1)
...
invisible(sapply(t:1,function(i) pp$points3d(m[,,i],col=i+1)))
