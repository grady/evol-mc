...
(1992).\footnote{\url{http://web.mit.edu/~wingated/www/introductions/mcmc-gibbs-intro.pdf}}
Here we find Baysean location-scale parameter estimates for a
t-distribution likelihood with known degrees of freedom $5/2$. The
model specification in this case is $y|\mu,w\sim N(\mu,w^{-1})$ and
...
posterior <- function(state){
  ...
  (apply(state,1,f) + dnorm(state[,1],sd=100,log=TRUE) +
